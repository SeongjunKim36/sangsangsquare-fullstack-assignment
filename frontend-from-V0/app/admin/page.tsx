...
import {
  mockAdminMeetings,
  mockApplicantsByMeeting,
} from "@/lib/admin-mock-data";
import {
  formatDateKorean,
  getRelativeTime,
  isAnnouncementPassed,
} from "@/lib/date-utils";
...
export default function AdminPage() {
  ...
  const handleSelectMeeting = (meetingId: number) => {
    ...
    setTimeout(() => {
      const applicantList = mockApplicantsByMeeting[meetingId] || [];
  ...
  const handleCreateMeeting = () => {
    ...
    if (!formCapacity || parseInt(formCapacity) < 1) {
    ...
    // Mock API 호출
    setTimeout(() => {
      const newMeeting: AdminMeetingItem = {
        id: Date.now(),
      ...
      mockApplicantsByMeeting[newMeeting.id] = [];
  ...
  const handleUpdateStatus = () => {
    ...
    if (!applicant || !selectedMeeting) return;
    ...
    // 선정 시 정원 체크
    if (action === "select") {
      const currentSelected = selectedMeeting.selectedCount;
      // 만약 현재 SELECTED에서 변경하는 경우 (재변경) 정원 체크 제외
      if (
        applicant.status !== ApplicationStatus.SELECTED &&
        currentSelected >= selectedMeeting.capacity
      ) {
        toast.error("모집 인원을 초과할 수 없습니다.");
    ...
    setMeetings((prev) =>
      prev.map((m) => {
        ...
        let { selectedCount, rejectedCount, pendingCount } = m;

        // 이전 상태에서 카운트 감소
        if (applicant.status === ApplicationStatus.SELECTED) selectedCount--;
        else if (applicant.status === ApplicationStatus.REJECTED)
          rejectedCount--;
        else if (applicant.status === ApplicationStatus.PENDING) pendingCount--;

        // 새 상태에서 카운트 증가
        if (newStatus === ApplicationStatus.SELECTED) selectedCount++;
        else if (newStatus === ApplicationStatus.REJECTED) rejectedCount++;
        else if (newStatus === ApplicationStatus.PENDING) pendingCount++;

        return { ...m, selectedCount, rejectedCount, pendingCount };
      })
    );

    toast.success(
      `${applicant.applicantName}님이 ${action === "select" ? "선정" : "탈락"} 처리되었습니다.`
    );
    setConfirmDialog({ open: false, applicant: null, action: "select" });
  };
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormAnnouncementAt(e.target.value)}
                  />
                </div>
                  ...
                  {selectedMeeting ? (
                    <>
                      ...
                      <span className="ml-auto text-sm font-normal text-muted-foreground">
                        정원 {selectedMeeting.selectedCount}/
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {confirmDialog.action === "select" ? "선정" : "탈락"}
              하시겠습니까?
            </DialogDescription>
          </DialogHeader>
                ...
                isPassed ? (
                  <div className="flex justify-end gap-2">
                    <Tooltip>
                      ...
                      {isCapacityFull && (
                        <TooltipContent>정원이 가득 찼습니다</TooltipContent>
                      )}
                    </Tooltip>
                    <Button
                      size="sm"
